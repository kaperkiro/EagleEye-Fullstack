...
class RtspStream:
    ...
    def connect(self, retries: int = 3, delay: int = 2) -> bool:
        ...
        while attempt < retries:
            try:
                logger.info(
                    f"Attempt {attempt + 1}/{retries} to connect to RTSP stream: {self.rtsp_url}"
                )
                ...
                for i in range(30):  # Try up to 30 frames
                    ...
                    logger.debug(
                        f"Frame read attempt {i+1}: ret={ret}, frame={frame is not None}"
                    )
                ...
                if not ret or frame is None or frame.size == 0:
                    logger.error(
                        f"Cannot read valid frame from RTSP stream: {self.rtsp_url}"
                    )
                    raise Exception("No valid frames read after 30 attempts")

                logger.info(
                    f"Successfully connected to RTSP stream for camera {self.camera_id}"
                )
                return True
    ...
    def _frame_to_bytes(self, frame: np.ndarray) -> bytes:
        ...
        try:
            ...
            logger.error(
                f"Error converting frame to bytes for camera {self.camera_id}: {str(e)}"
            )
            return b""
        ...
        while True:
            try:
                if not self.capture or not self.capture.isOpened():
                    logger.warning(
                        f"Capture not open for camera {self.camera_id}, attempting reconnect"
                    )
                    if not self.connect():
                        break
                    continue
                ...
                if not ret or frame is None or frame.size == 0:
                    logger.warning(
                        f"Failed to grab frame for camera {self.camera_id}, skipping frame"
                    )
                    time.sleep(0.1)  # Brief pause before retry
                    continue

                logger.debug(
                    f"Frame {frame_count} read successfully for camera {self.camera_id}"
                )
                frame_with_detections = self._draw_detections(frame)
                cv2.imshow(self.window_name, frame_with_detections)
                logger.debug(
                    f"Displayed frame {frame_count} for camera {self.camera_id}"
                )
                ...
                if frame_count % 900 == 0:
                    ...
                    if thumbnail_bytes:
                        ...
                        logger.info(
                            f"Saved thumbnail as bytes for camera {self.camera_id}"
                        )
                ...
                logger.error(
                    f"Error in stream loop for camera {self.camera_id}: {str(e)}"
                )
                time.sleep(1)
                self._force_release()
                if not self.connect():
    ...
    def release(self) -> None:
        ...
        try:
            ...
            logger.error(
                f"Error releasing window for camera {self.camera_id}: {str(e)}"
            )
